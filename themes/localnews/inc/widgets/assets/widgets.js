...
jQuery(document).ready( function($) {
    function local_news_widgets_handler() {
        // multicheckbox field
        $( ".local-news-multicheckbox-field" ).on( "click, change", ".multicheckbox-content input", function() {
            var _this = $(this), parent = _this.parents( ".local-news-multicheckbox-field" ), currentVal, currentFieldVal = parent.find( ".widefat" ).val();
        ...
        // checkbox field
        $( ".local-news-checkbox-field" ).on( "click, change", "input", function() {
            var _this = $(this)
            if( _this.is(":checked") ) {
                _this.val( "1" )
            } else {
        ...
        // upload field
        $( ".local-news-upload-field" ).on( "click", ".upload-trigger", function(event) {
        ...
        // remove image
        $( ".local-news-upload-field" ).on( "click", ".upload-buttons .remove-image", function(event) {
            event.preventDefault();
            var _this = $(this);
        ...
        // icon text handler
        var iconTextContainer = $( ".local-news-icon-text-field" )
        ...
        function setIconTextFieldValue(el,iconEl,txtEl) {
            // console.log({icon: iconEl.data( "value" ), title: txtEl.val()})
            el.find( 'input.widefat[type="hidden"]' ).val(JSON.stringify( {icon: iconEl.data( "value" ), title: txtEl.val()} ))
        }
    }
    local_news_widgets_handler();
